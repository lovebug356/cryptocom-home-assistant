...
async def test_basic_properties():
    ...
    sensor = LastPriceSensor(cryptocom_data, MarketSymbol.try_from_string("BTC/USD"))
    assert sensor.unit_of_measurement == '$'
    assert sensor.name == 'btc_usd_last_price'

    sensor = LastPriceSensor(cryptocom_data, MarketSymbol.try_from_string("BTC/EUR"))
    assert sensor.unit_of_measurement == '€'

    sensor = LastPriceSensor(cryptocom_data, MarketSymbol.try_from_string("BTC/USDT"))
